...
import { type Token, getTokensByChain, COMMON_TOKENS } from '@/lib/constants/tokens'
...
export function TokenSelector({
  ...
  const commonTokens = availableTokens.filter(token =>
    Object.values(COMMON_TOKENS).includes(token.symbol as any)
  )
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="bg-background border-border max-w-md">
        ...
        <div className="space-y-4">
          ...
          {/* Common Tokens */}
          {!searchQuery && commonTokens.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-sm font-medium text-muted-foreground">Common Tokens</h4>
              <div className="flex flex-wrap gap-2">
                {commonTokens.slice(0, 4).map((token) => (
                  <TokenQuickSelect
                    key={token.address}
                    token={token}
...
function TokenQuickSelect({
  ...
  return (
    <Button
      ...
      className="h-8 bg-muted/30 border-border/50 hover:bg-muted/50"
    >
